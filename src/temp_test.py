...
norm_mean_skeleton = np.load('./prepared_data/T1_Skelet.npy')
...
def main(args):
    ...
    with open(args.datapathds, 'rb') as file_pi:
         ds = pickle.load(file_pi)
    with open(args.datapathfull, 'rb') as file_pi:
    ...
    if args.att:
        ...
        model.load_state_dict(torch.load(f'./weights/model_{args.modality}_att', map_location='cpu')['model_weights'])
    else:
        model = hg(num_stacks=args.stacks, num_blocks=args.blocks, num_classes=args.njoints)
        model = torch.nn.DataParallel(model).to(device)
        model.load_state_dict(torch.load(f'./weights/model_simple_{args.modality}_att', map_location='cpu')['model_weights'])
...
if __name__ == "__main__":
    ...
    ## Parameters
    parser.add_argument('--datapathds', default='./prepared_data/prepared_testset_t1_ds', type=str,
                        help='Dataset address')     
    parser.add_argument('--datapathfull', default='./prepared_data/prepared_testset_t1_full', type=str,
                        help='Dataset address')                           
